...
(function() {
  ...
  formatMessage = function(message, args) {
    ...
    if (message.indexOf('%s') >= 0) {
      ...
      return message.replace(/%e/g, function(match) {
  ...
  ComposedField = (function(_super) {
    __extends(ComposedField, _super);

    function ComposedField(elm, options) {
      ComposedField.__super__.constructor.call(this, elm, options);
    }

    ComposedField.prototype.getComponents = function() {
      var components, field, fields, _i, _len;
      components = [];
      fields = this.element.data("composed").split(',');
    ...
    ComposedField.prototype.getValue = function() {
      var value;
      value = _.map(this.getComponents(), function(x) {
        return x.val();
      }).join(this.element.data("composed-joiner"));
      this.element.val(value);
      return value;
    };

    ComposedField.prototype.unbindEvents = function() {
      var component, _i, _len, _ref, _results;
      _ref = $(this.getComponents());
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        component = _ref[_i];
        _results.push(component.off("." + this.id));
      }
      return _results;
    };

    ComposedField.prototype.bindEvents = function() {
      var component, trigger, _i, _len, _ref, _results;
      this.unbindEvents();
      trigger = this.element.data("trigger");
      _ref = $(this.getComponents());
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (trigger !== "change") {
          _results.push(component.on("change." + this.id, _.bind(this.eventValidate, this)));
        } else {
          _results.push(void 0);
        }
      }
  ...
  Form = (function() {
    ...
    Form.prototype.initializeFields = function() {
      var composedField, element, field, fieldElm, _i, _j, _len, _len1, _ref, _ref1, _results;
      this.fields = [];
      this.fieldsByName = {};
      ...
      _ref1 = this.element.find('[data-composed]');
      _results = [];
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
        composedField = _ref1[_j];
